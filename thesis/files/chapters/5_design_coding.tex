...
Per ogni coppia \texttt{Job-Printer} viene chiamato il metodo \texttt{\_\_create\_task\_variables} il quale si occupa della effettiva creazione dell'oggetto di tipo \texttt{TaskVariable}.
\subsubsection{Implementazione}
...
Tale metodo si occupa quindi della costruizione della soluzione orchestrando le chiamate ai metodi \texttt{\_\_get\_status\_string}, \texttt{\_\_get\_printer\_schedule} e \texttt{\_\_update\_job\_start\_times} i quali si occupano,
...
\begin{itemize}
    ...
    \item Il \textit{client} può inserire nella intestazione della richiesta (detta anche \textit{header}) il \textit{JSON Web Token} ricevuto;
...
Il test si considera superato se il modulo pianifica correttamente l'ordine senza errori e minimizzando i numeri di cambio materiale.
\subsubsection{Implementazione}
...
\subsubsection{Scopo}
Questo test ha lo scopo di validare la capacità di due ordini di produzione di cui uno urgente.
...
\subsubsection{Scopo}
Questo test ha lo scopo di validare la capacità di un ordine parziale.
