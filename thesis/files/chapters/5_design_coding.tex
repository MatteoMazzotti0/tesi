...
\begin{itemize}
    \item La tabella \textit{scheduling\_results} così formata:
    \begin{itemize}
    \item \textit{id}: intero auto-incrementale usato come chiave primaria;
    \item \textit{user\_id}: chiave esterna verso la tabella \textit{users} contenente i dati degli utenti registrati;
    \item \textit{data}: campo \textit{json} obbligatorio che memorizza il payload completo della soluzione;
    \item \textit{is\_accepted}: boolean obbligatorio valorizzato di default a \textit{false} per indicare l'approvazione dell'utente;
    \item \textit{created\_at}: campo di tipo \textit{DateTime} per tracciare la data di ricezione del risultato;
    \item \textit{updated\_at}: campo di tipo \textit{DateTime} per tracciare la data dell'ultima modifica risultato.
    \end{itemize}
    \item La tabella \textit{scheduling\_results\_orders} così formata:
    \begin{itemize}
        \item \textit{id}: intero auto-incrementale usato come chiave primaria;
        \item \textit{order\_id}: chiave esterna verso la tabella \textit{orders} contenente gli ordini ricevuti;
        \item \textit{scheduling\_result\_id}: chiave esterna verso \textit{scheduling\_results}.
    \end{itemize}
\end{itemize}
